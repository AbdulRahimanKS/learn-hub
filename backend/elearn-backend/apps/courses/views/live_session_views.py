...
class LiveSessionListCreateView(APIView):
    ...
    def get(self, request, batch_id):
        try:
            ...
            if is_student:
                if not BatchEnrollment.objects.filter(batch_id=batch_id, student=request.user, status__in=[BatchEnrollment.Status.ACTIVE, BatchEnrollment.Status.COMPLETED]).exists():
                    raise ServiceError(detail="Access denied. You are not an active or completed student in this batch.", status_code=status.HTTP_403_FORBIDDEN)
            ...
            serializer = LiveSessionSerializer(qs, many=True, context={'request': request})
            return format_success_response(message="Live sessions retrieved successfully", data=serializer.data)

        except ServiceError:
            raise
        except Exception as e:
            logger.error(f"Error listing live sessions for batch {batch_id}: {str(e)}")
    ...
    def post(self, request, batch_id):
        try:
            ...
            logger.error(f"Error creating live session for batch {batch_id}: {str(e)}")
            raise ServiceError(detail=str(e), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class LiveSessionDetailView(APIView):
    ...
    def get(self, request, batch_id, session_id):
        try:
            ...
            logger.error(f"Error retrieving live session for batch {batch_id} and session {session_id}: {str(e)}")
            raise ServiceError(detail=str(e), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def patch(self, request, batch_id, session_id):
        try:
            ...
            logger.error(f"Error updating live session for batch {batch_id} and session {session_id}: {str(e)}")
            raise ServiceError(detail=str(e), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def delete(self, request, batch_id, session_id):
        try:
            ...
            logger.error(f"Error deleting live session for batch {batch_id} and session {session_id}: {str(e)}")
            raise ServiceError(detail=str(e), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
