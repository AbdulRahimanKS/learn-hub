    ...
    <Dialog open={open} onOpenChange={(v) => !v && !isSubmitting && onClose()}>
      ...
      <DialogContent className="flex h-[95vh] sm:h-[90vh] w-[98vw] sm:w-[95vw] max-w-[1200px] flex-col overflow-hidden rounded-2xl sm:rounded-[28px] border border-primary/25 bg-background p-0 text-foreground shadow-2xl [&>button]:right-4 [&>button]:top-4 [&>button]:z-[60] [&>button]:text-primary-foreground [&>button]:opacity-90 [&>button]:ring-offset-transparent [&>button]:hover:bg-white/10 [&>button]:hover:text-primary-foreground [&>button]:hover:opacity-100 data-[state=open]:[&>button]:bg-transparent data-[state=open]:[&>button]:text-primary-foreground">
        ...
        <div className="relative flex-none overflow-hidden border-b border-white/10 gradient-primary px-4 sm:px-6 pb-3 sm:pb-4 pt-5 sm:pt-7 text-primary-foreground">
          ...
          <div className="relative mb-4 sm:mb-6 flex flex-col gap-3 sm:gap-5">
            <div>
              ...
              <DialogTitle className="mb-1 text-lg sm:text-2xl font-bold tracking-tight text-primary-foreground">{test.title}</DialogTitle>
        ...
        <div className="flex flex-col lg:flex-row min-h-0 flex-1 overflow-hidden">
           ...
           {/* Left: Question Area */}
           <div className="relative min-h-0 flex-1 overflow-y-auto bg-background px-4 sm:px-10 py-5 sm:py-8 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent">
              <div className="max-w-[800px] mb-8">
                {/* Question Text — number & marks live in the Question Navigator */}
                <h2 className="mb-5 whitespace-pre-line text-base sm:text-lg font-semibold leading-relaxed text-foreground md:text-xl">
                  {activeQuestion.text}
                </h2>
                ...
                <div className="space-y-4 mb-10">
                  ...
                  <div className="overflow-hidden rounded-md border border-input bg-background transition-all focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 focus-within:ring-offset-background">
                    <Textarea
                      ...
                      className="min-h-[150px] sm:min-h-[200px] w-full resize-none rounded-none border-0 bg-transparent p-4 sm:p-6 text-sm sm:text-[16px] leading-relaxed text-foreground shadow-none placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-0"
                    />
                  </div>
                </div>
                ...
                <div className="grid gap-2">
                  ...
                  <div
                    ...
                    className={cn(
                      'rounded-lg border-2 border-dashed p-4 sm:p-8 text-center transition-colors',
           ...
           <div className="flex flex-col lg:w-[320px] lg:flex-none border-t lg:border-t-0 lg:border-l border-border bg-background/40 p-4 sm:p-6 max-h-[350px] lg:max-h-none min-h-0 shrink-0 lg:shrink whitespace-normal">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent">
                <div className="flex flex-wrap gap-2 lg:flex-col lg:gap-1.5">
                  {test.questions.map((q, i) => {
                    const isAnswered = (answers[q.id] && answers[q.id].trim() !== '') || files[q.id];
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "relative flex items-center transition-all border rounded-xl",
                          // Mobile: Circular/Square grid style
                          "w-11 h-11 justify-center shrink-0",
                          // Desktop: Full width list style
                          "lg:w-full lg:h-auto lg:px-4 lg:py-3.5 lg:justify-between lg:border-l-[3px] lg:rounded-lg",
                          isActive 
                            ? "border-primary bg-primary/10 lg:bg-primary/5 lg:border-l-primary shadow-sm" 
                            : isAnswered
                              ? "border-success/30 bg-success/5 hover:border-success/50"
                              : "border-border/50 bg-background/20 hover:bg-accent/50",
                          !isActive && !isAnswered && "text-muted-foreground"
                        )}
                      >
                        {/* Mobile view content */}
                        <div className="flex flex-col items-center lg:hidden">
                          <span className={cn(
                            "text-sm font-bold",
                            isActive ? "text-primary" : isAnswered ? "text-success" : "text-foreground"
                          )}>
                            {i + 1}
                          </span>
                          {isAnswered && !isActive && (
                            <div className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-success rounded-full border-2 border-background flex items-center justify-center">
                        ...
                        <div className="hidden lg:flex flex-1 items-center justify-between min-w-0">
                          <span className={cn("text-[14px] font-semibold truncate", isActive ? 'text-primary' : 'text-foreground')}>
                            Question {i + 1}
                          </span>
                          <div className="flex items-center gap-3">
                            ...
                            {isAnswered ? (
                              <CheckCircle className={cn("h-4 w-4 shrink-0", isActive ? 'text-primary' : 'text-success')} />
                            ) : (
        ...
        {/* Footer Navigation */}
        <div className="flex-none flex items-center justify-between border-t border-border bg-background px-4 sm:px-8 py-3 sm:py-5">
          <Button 
            variant="ghost" 
          ...
          <div className="flex h-10 sm:h-12 items-stretch gap-1.5 sm:gap-2">
            {activeQuestionIndex < totalQuestions - 1 ? (
              <Button
                variant="gradient"
                className="h-full rounded-lg sm:rounded-xl px-4 sm:px-8 text-xs sm:text-sm font-bold shadow-none hover:shadow-none"
                onClick={() => setActiveQuestionIndex(p => p + 1)}
              >
                Next <ArrowRight className="ml-1 sm:ml-2 h-3.5 w-3.5 sm:h-4 sm:w-4" />
              </Button>
            ) : (
              <>
                <Button
                ...
                >
                  {isSubmitting ? (
                    ...
                    <div className="mr-2 h-2.5 w-2.5 sm:mr-2.5 sm:h-3.5 sm:w-3.5 rounded-full border-[1.5px] border-white/60" />
                  )}
